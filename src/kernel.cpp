...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
	if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zHSS stake block is less than allowed start height", __func__);

	if (CZHssStake* zHSS = dynamic_cast<CZHssStake*>(stake)) {
		CBlockIndex* pindexFrom = zHSS->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zHSS stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zHSS stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zHSS->GetValue()));
		if (nChecksum200 != zHSS->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zHSS->GetChecksum(), nChecksum200);
	}
	else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
	...
	if (txin.IsZerocoinSpend()) {
		...
		stake = std::unique_ptr<CStakeInput>(new CZHssStake(spend));

		if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
			return error("%s: staked zHSS fails context checks", __func__);
	}
	else {
		...
		CHssStake* hssInput = new CHssStake();
		hssInput->SetInput(txPrev, txin.prevout.n);
		stake = std::unique_ptr<CStakeInput>(hssInput);
	}
	...
	if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zHSS is checked above in ContextualCheckZerocoinStake()

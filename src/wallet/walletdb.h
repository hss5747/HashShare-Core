...
public:
    ...
    bool WriteZHSSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHSSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHSSSeed_deprecated(uint256& seed);
    bool EraseZHSSSeed();
    bool EraseZHSSSeed_deprecated();

    bool WriteZHSSCount(const uint32_t& nCount);
    bool ReadZHSSCount(uint32_t& nCount);

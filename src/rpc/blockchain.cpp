...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zhssObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zhssObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zhssObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zHSSsupply", zhssObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zHSSsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zHSS denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zHSS denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zHSS denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zHSS denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zHSS denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zHSS denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zHSS denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zHSS denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zHSS denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in hashsharecoin\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of hashsharecoin addresses\n"
            "     \"hashsharecoinaddress\"   	 	(string) hashsharecoin address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZHSS_FAILED_ACCUMULATOR_INITIALIZATION);

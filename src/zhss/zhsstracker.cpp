...
CzHSSTracker::CzHSSTracker(std::string strWalletFile)
{
...
CzHSSTracker::~CzHSSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzHSSTracker::Init()
{
...
bool CzHSSTracker::Archive(CMintMeta& meta)
{
...
bool CzHSSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzHSSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzHSSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzHSSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzHSSTracker::GetSpendCache(const uint256& hashStake)
{
...
std::vector<uint256> CzHSSTracker::GetSerialHashes()
{
...
CAmount CzHSSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzHSSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzHSSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzHSSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzHSSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzHSSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzHSSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzHSSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzHSSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzHSSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzHSSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzHSSWallet* zHSSWallet)
{
    bool iszHSSWalletInitialized = (NULL != zHSSWallet);
    CMintMeta meta;
    ...
    if (! iszHSSWalletInitialized)
        zHSSWallet = new CzHSSWallet(strWalletFile);
    meta.isSeedCorrect = zHSSWallet->CheckSeed(dMint);
    if (! iszHSSWalletInitialized)
        delete zHSSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzHSSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHSSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzHSSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzHSSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzHSSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzHSSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzHSSWallet* zHSSWallet = new CzHSSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zHSSWallet);
        }
        delete zHSSWallet;
        LogPrint("zero", "%s: added %d dzhss from DB\n", __func__, listDeterministicDB.size());
    }

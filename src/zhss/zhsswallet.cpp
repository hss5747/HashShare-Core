...
CzHSSWallet::CzHSSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhss seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHSSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHSSSeed_deprecated()) {
                    LogPrintf("%s: Updated zHSS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhss seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhss wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHSSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHSSCount(nCountLastUsed);
    else if (!walletdb.ReadZHSSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHSSWallet::Lock()
{
    seedMaster = 0;
}

void CzHSSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHSSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHSS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHSSWallet::LoadMintPoolFromDB()
{
...
void CzHSSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHSSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHSSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhssTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHSSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHSS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhssTracker which also adds to database
    pwalletMain->zhssTracker->Add(dMint, true);
...
void CzHSSWallet::SeedToZHSS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHSSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHSSWallet::GenerateDeterministicZHSS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHSSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHSSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzHSSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

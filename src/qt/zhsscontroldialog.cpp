...
#include "ui_zhsscontroldialog.h"
...
std::set<std::string> ZHssControlDialog::setSelectedMints;
std::set<CMintMeta> ZHssControlDialog::setMints;

bool CZHssControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZHssControlDialog::COLUMN_DENOMINATION || column == ZHssControlDialog::COLUMN_VERSION || column == ZHssControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZHssControlDialog::ZHssControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZHssControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZHssControlDialog::~ZHssControlDialog()
{
    delete ui;
}

void ZHssControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHssControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZHssControlWidgetItem* itemDenom(new CZHssControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZHssControlWidgetItem *itemMint = new CZHssControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zhssTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zhssTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zHSS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zHSS seed used to mint this zHSS is not the same as currently hold in the wallet";
            else
...
void ZHssControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHssControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZHss_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHssControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZHssControlDialog::GetSelectedMints()
{
...
void ZHssControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

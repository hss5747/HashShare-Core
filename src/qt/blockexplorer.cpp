...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::HSS, nValue);
...
{
    std::string genesisblockhash = "000008948ad86e610b4b29d8627e4490938f3886e1c5ea2b2de5831573656bc5";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (hashsharecoin.conf).");
            QMessageBox::warning(this, "HashShare Coin Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#f8f6f6; bgcolor:#0d8adc;}\n a, span { font-family: monospace; }\n span.addr {color:#0d8adc; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #0d8adc;}\n td.d0 {font-weight: bold; color:#f8f6f6;}\n h2, h3 { white-space:nowrap; color:#0d8adc;}\n a { color:#88f6f6; text-decoration:none; }\n a.nav {color:#0d8adc;}\n";

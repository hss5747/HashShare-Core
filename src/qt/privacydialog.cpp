                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zHSS ought to be enough for anybody." - Bill Gates, 2017
    ui->zHSSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzHSSSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zHSSpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzHSS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHSS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zHSS, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zHSS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zHSS in ") +
...
void PrivacyDialog::on_pushButtonSpendzHSS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHSS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zHSS, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zHSS
        sendzHSS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zHSS
    sendzHSS();
}

void PrivacyDialog::on_pushButtonZHssControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZHssControlDialog* zHssControl = new ZHssControlDialog(this);
    zHssControl->setModel(walletModel);
    zHssControl->exec();
}

void PrivacyDialog::setZHssControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzHssSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzHSS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid HashShareCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zHSSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zHSS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " HSS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zHSS</b>";
    ...
    // use mints from zHSS selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZHssControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZHssControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zHSS transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zHSS was spent successfully update the addressbook with the label
    ...
    // Clear zhss selector in case it was used
    ZHssControlDialog::setSelectedMints.clear();
    ui->labelzHssSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHSS Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Hss, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zHSS Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zhssTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zHSS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zHSS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zHSS "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zHSS "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zHSS </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zHSS </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zHSS </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzHSSSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzHSS->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zHSS
        //ui->pushButtonMintzHSS->setEnabled(false);
        //ui->pushButtonMintzHSS->setToolTip(tr("zHSS is currently disabled due to maintenance."));

        // Spend zHSS
        ui->pushButtonSpendzHSS->setEnabled(false);
        ui->pushButtonSpendzHSS->setToolTip(tr("zHSS is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zHSS
        //ui->pushButtonMintzHSS->setEnabled(true);
        //ui->pushButtonMintzHSS->setToolTip(tr("PrivacyDialog", "Enter an amount of HSS to convert to zHSS", 0));

        // Spend zHSS
        ui->pushButtonSpendzHSS->setEnabled(true);
        ui->pushButtonSpendzHSS->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}

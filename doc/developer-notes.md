...
- [Development guidelines](#development-guidelines)
    - [General HashShare Coin](#general-hashsharecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
HashShare Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play HSSs (tHSS)" on the test network, if you
...
HashShare Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/hashsharecoin-project/hashsharecoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_hashsharecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_hashsharecoin --log_level=test_suite
$ valgrind -v --leak-check=full src/hashsharecoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_hashsharecoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates HSSs (if wallet is enabled).
...
However, in open source software such as HashShare Coin, where everyone uses
...
pay attention to for reviewers of HashShare Coin code.

General HashShare Coin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: HashShare Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against HashShare Coin so that they can be integrated
...
file descriptors it will cause problems with HashShareCoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:HashShareCoin-Project/HashShareCoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `hashsharecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `hashsharecoin-cli` and the GUI debug console use this table to determine how to
